...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        if input_fn is not None:
            ...
            try:
                ...
                # Closed or invalid stdin is not a user decline. Never persist
                # a decision that nobody actually made.
                print()
                logger.info(
                ...
                # Aborted, not declined — Ctrl+C keeps its own message and
                # must not create the durable skip marker.
                print()
                print("  Cancelled.")
                return False
    ...
    if origin_ahead > 0:
        ...
        # The official comparison completed successfully even though no
        # automatic sync was safe. Callers may truthfully avoid claiming the
...
def _warn_pending_fleet_restart_on_startup() -> None:
    ...
    except Exception:  # noqa: S110 -- startup hint is explicitly best-effort
        pass
...
def _run_pending_fleet_restart() -> bool:
    ...
    except Exception:  # noqa: S110 -- stale-module cleanup must not block restart
        pass
    try:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    except Exception:  # noqa: S110 -- receipt telemetry must not block relaunch
        pass
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if socket_acks:
        ...
        except Exception:  # noqa: S110 -- malformed remote ACK uses local timeout
            pass
        print(
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _serve_entries:
                ...
                except Exception:  # noqa: S110 -- receipt telemetry must not block pause
                    pass
    ...
    try:
        ...
        if isinstance(_windows_gateway_resume, dict):
            ...
            except Exception:  # noqa: S110 -- receipt telemetry must not mask update
                pass
