...
async def update_hermes():
    ...
    return {
        ...
        "status": "spawned",
        # No "action_id" here: _spawn_durable_action generates its own
        # internal action_id (timestamp-based, recorded via
        # _write_action_record for restart-independent tracking) rather
        # than the secrets.token_hex(16)/HERMES_ACTION_ID scheme
        # _spawn_hermes_action uses for other dashboard actions — the two
        # are not interchangeable, so exposing the unused token here would
        # name an ID nothing was actually recorded against.
    }
