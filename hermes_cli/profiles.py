...
# not profile data" consumed by hermes_cli.profile_distribution.USER_OWNED_EXCLUDE
# (layers its distribution-specific additions on top). Restored after upstream's
# clone-all-specific split (_CLONE_ALL_DEFAULT_EXCLUDE_ROOT / _CLONE_ALL_HISTORY_
# EXCLUDE_ROOT below) dropped the public name profile_distribution.py still needs;
# kept in full, including the credential entries the narrower clone-all constants
# do not carry, so distribution exclusion does not regress.
